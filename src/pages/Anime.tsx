...
const Anime = () => {
  ...
  async function getImages(){
   ...
   try{
     const promis = await fetch(apiUrl+`?q=anime&categories=111&purity=110&sorting=relevance&order=desc&ai_art_filter=0`)
  ...
  async function getOtherPages(){
   ...
   try{
    ...
    setPageNum((prev)=>prev+1)
     ...
     const promis = await fetch(apiUrl+`?q=anime&categories=111&purity=110&sorting=relevance&order=desc&ai_art_filter=0&page=${pageNum}`)
    const imagesList = await promis.json()
